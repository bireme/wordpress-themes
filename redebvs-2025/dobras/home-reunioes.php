...
// Limites / cache
$rss_limit      = 3;
$rss_cache_secs = 10 * MINUTE_IN_SECONDS;
 ...
 * ]
 */
if ( ! function_exists('bvs_reunioes_get_rss_items') ) {
    function bvs_reunioes_get_rss_items($rss_url, $limit = 3, $cache_secs = 600, &$debug_out = '') {
        ...
        if ( $cache_secs > 0 ) {
            ...
            if ( is_array($cached) && ! empty($cached) ) {
                $debug[] = 'cache hit (' . count($cached) . ' items)';
                $debug_out = implode(' | ', $debug);
                return $cached;
            }
        }
        // Limpa transient anterior que pode estar vazio
        delete_transient($cache_key);

        // Monta headers — inclui Basic Auth se a URL for do domínio de teste
        $headers = [
            'Accept'     => 'application/rss+xml, application/xml, text/xml;q=0.9,*/*;q=0.8',
            'User-Agent' => 'WordPress/' . get_bloginfo('version') . '; ' . home_url('/'),
        ];
        if ( strpos($rss_url, 'red.teste.bvsalud.org') !== false ) {
            $headers['Authorization'] = 'Basic ' . base64_encode('admin-tst:B1r@202601#');
        }
