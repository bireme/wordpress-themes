...
class search_bvs_widget extends WP_Widget {
    ...
    function widget( $args, $instance ) {
        ...
        $tooltip = "<h4><span class='fas fa-lightbulb'></span>". __('Dica de pesquisa', 'bvs_lang') ."</h4>
                    <p>". __('Para buscar por palavras do t&iacute;tulo utilize', 'bvs_lang') ."</p>
        ...
        <form style="display: none;" class="default-search-form" role="search" action="https://pesquisa.bvsalud.org/aps/" method="get" id="defaultSearchForm" onsubmit="if(defaultSearchForm.s.value=='<?php _e('Pesquisar', 'bvs_lang'); ?>') defaultSearchForm.s.value = '';">
            <?php if( function_exists('pll_current_language') ){ ?>
            <input type="hidden" name="lang" value="<?php echo pll_current_language(); ?>" />
