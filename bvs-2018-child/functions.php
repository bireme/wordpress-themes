...
function wp_bootstrap_starter_widgets_init() {
    ...
    register_sidebar( array(
        'name'          => esc_html__( 'Acesso Nucleos', 'wp-bootstrap-starter' ),
        'id'            => 'acesso_nucleos',
        'description'   => esc_html__( 'Add widgets here.', 'wp-bootstrap-starter' ),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget'  => '</section>',
    ...
    register_sidebar( array(
        'name'          => esc_html__( 'Atalho para Noticias', 'wp-bootstrap-starter' ),
        'id'            => 'atalho_noticias',
        'description'   => esc_html__( 'Add widgets here.', 'wp-bootstrap-starter' ),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget'  => '</section>',
...
function wp_bootstrap_starter_scripts() {
	// load bootstrap css
	wp_enqueue_style( 'wp-bootstrap-starter-bootstrap-css', get_stylesheet_directory_uri() . '/inc/assets/css/bootstrap.min.css' );
    // fontawesome cdn
    ...
    if(get_theme_mod( 'theme_option_setting' ) && get_theme_mod( 'theme_option_setting' ) !== 'default') {
        wp_enqueue_style( 'wp-bootstrap-starter-'.get_theme_mod( 'theme_option_setting' ), get_stylesheet_directory_uri() . '/inc/assets/css/presets/theme-option/'.get_theme_mod( 'theme_option_setting' ).'.css', false, '' );
    }
    ...
    if(get_theme_mod( 'preset_style_setting' ) && get_theme_mod( 'preset_style_setting' ) !== 'default') {
        wp_enqueue_style( 'wp-bootstrap-starter-'.get_theme_mod( 'preset_style_setting' ), get_stylesheet_directory_uri() . '/inc/assets/css/presets/typography/'.get_theme_mod( 'preset_style_setting' ).'.css', false, '' );
    }
    //Color Scheme
    /*if(get_theme_mod( 'preset_color_scheme_setting' ) && get_theme_mod( 'preset_color_scheme_setting' ) !== 'default') {
        wp_enqueue_style( 'wp-bootstrap-starter-'.get_theme_mod( 'preset_color_scheme_setting' ), get_stylesheet_directory_uri() . '/inc/assets/css/presets/color-scheme/'.get_theme_mod( 'preset_color_scheme_setting' ).'.css', false, '' );
    }else {
        wp_enqueue_style( 'wp-bootstrap-starter-default', get_stylesheet_directory_uri() . '/inc/assets/css/presets/color-scheme/blue.css', false, '' );
    }*/
	...
	wp_enqueue_script( 'wp-bootstrap-starter-skip-link-focus-fix', get_stylesheet_directory_uri() . '/inc/assets/js/skip-link-focus-fix.min.js', array(), '20151215', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}

    //import styles child theme
    //custom theme style
    wp_enqueue_style( 'child-style', get_stylesheet_directory_uri() . '/assets/css/style.css', array( 'twentytwelve-style' ) );

    //custom script
    wp_enqueue_script( 'custom-script', get_stylesheet_directory_uri() .'/assets/js/scripts.js', array('jquery') );
}
 ...
 */
require get_stylesheet_directory() . '/inc/plugin-compatibility/plugin-compatibility.php';
...
//--- language child theme ---//
add_action ('after_setup_theme', 'child_spacious_setup');
function child_spacious_setup () {
    load_child_theme_textdomain ('bvs_lang', get_stylesheet_directory(). '/languages');
}

//import main setting theme
require "classes/main-setting.php";


//cut text for summary
function crop_text($text, $limit){
    if (strlen($text) <= $limit){
        echo $text;
    } 
    else{
        echo array_shift(explode('||', wordwrap($text, $limit, '||'))) . " [...]";
    }
}

//return default image
function get_default_img(){
    return "http://via.placeholder.com/520x270";
}

function get_default_icon_img(){
    return "http://via.placeholder.com/74x74";
}
