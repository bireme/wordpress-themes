...
.bvs-side{position:relative; border-right: 1px solid #00205c17;
    border-radius: 0;}
...
/* Layout grid */
    ...
    .bvs-countries .name, .bvs-group .body .name{    font-weight: 500 !important;
    ...
    /* Main (Tabela) */
    .bvs-main{    background: var(--panel);
    border-radius: 14px;
    padding: 8px;
    box-shadow: 0 1px 2px rgba(10, 20, 40, .05);

    overflow-x: scroll;background:var(--panel); border-radius:14px; padding:8px; box-shadow:0 1px 2px rgba(10,20,40,.05)}
    ...
    .bvs-col-title a{    color: var(--accent);
    ...
    /* Tiny helpers */
    .badge{display:inline-block; padding:2px 8px; border-radius:999px; background:var(--chip); color:#194a91; font-size:12px; font-weight:700}
...
<div class="bvs-wrap">
  ...
  <aside class="bvs-side" id="bvs-filters">
    ...
    <div class="bvs-search" style="margin:10px 0 12px;">
      <input id="bvs-q" type="search" placeholder="Pesquise por título ou códigos" aria-label="Pesquisar">
      <span class="icon">
...
(function(){
  ...
  function renderCountries(){
    ...
    S.countriesFacet.forEach(item=>{
      ...
      row.addEventListener("click", ()=>{ S.countryRaw=item.raw; S.page=1; renderAll(); });
  ...
  function renderAssunto(){
    ...
    rowAll.addEventListener("keydown", e=>{ if(e.key==='Enter'||e.key===' '){ e.preventDefault(); rowAll.click(); }});
  ...
  document.getElementById("bvs-assunto-toggle").addEventListener("click", ()=>{
    S.assuntoOpen = !S.assuntoOpen; renderAssunto();
  });

  // ===== Filtragem local (país + busca) =====
  function filteredDocs(){
    ...
    if (S.search) {
      ...
      list = list.filter(d => {
        ...
        return title.includes(q) || editor.includes(q) || cc.includes(q);
      });
    }
    return list;
  }
  ...
  async function init(){
    // eventos fixos
    $q.addEventListener("input", e=>{ S.search = e.target.value.trim(); S.page=1; renderTable(); });
    $totalChip.addEventListener("click", ()=>{ S.countryRaw=null; S.page=1; renderAll(); });
    $totalChip.addEventListener("keydown", e=>{ if(e.key==='Enter'||e.key===' '){ e.preventDefault(); $totalChip.click(); }});
    ...
    if($page && $toggle){
      ...
      if(saved){ $page.classList.add("is-collapsed"); $toggle.setAttribute("aria-expanded","false"); $toggle.dataset.dir="left"; }
